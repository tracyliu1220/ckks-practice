...
Ciphertext Ciphertext::operator*(const Ciphertext & rhs) {
    ...
    mpz_class cur_Q = ret.Q;
    int cur_Q_bits = ret.Q_bits;

    vector<mpz_class> d0 = polynomial_times(c0, rhs.c0, cur_Q);
    vector<mpz_class> d1_0 = polynomial_times(c0, rhs.c1, cur_Q);
    vector<mpz_class> d1_1 = polynomial_times(c1, rhs.c0, cur_Q);
    vector<mpz_class> d1 = polynomial_add(d1_0, d1_1, cur_Q);
    vector<mpz_class> d2 = polynomial_times(c1, rhs.c1, cur_Q);

    for (int i = 0; i < (int)d2.size(); i++) {
        mpf_class d2_f(d2[i], cur_Q_bits + 64);
        d2[i] = (mpz_class)(((d2_f / P) * 10 + 5) / 10) % cur_Q;
    }

    vector<mpz_class> P0 = polynomial_times(d2, evaluation_key.first, P * cur_Q);
    vector<mpz_class> P1 = polynomial_times(d2, evaluation_key.second, P * cur_Q);

    ret.c0 = polynomial_add(d0, P0, cur_Q);
    ret.c1 = polynomial_add(d1, P1, cur_Q);
